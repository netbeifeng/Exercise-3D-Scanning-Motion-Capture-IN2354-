...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        const T _pose_0 = pose[0];
        const T _pose_1 = pose[1];
        const T _pose_2 = pose[2];
        const T _pose_3 = pose[3];
        const T _pose_4 = pose[4];
        const T _pose_5 = pose[5];
        //auto p_pose = PoseIncrement<double>(_pose);

        T incrementArray[6];
        incrementArray[0] = _pose_0;
        incrementArray[1] = _pose_1;
        incrementArray[2] = _pose_2;
        incrementArray[3] = _pose_3;
        incrementArray[4] = _pose_4;
        incrementArray[5] = _pose_5;
...
private:
    ...
    Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            A.row(idx) << a1, a2, a3, nx, ny, nz;
            b.row(idx) << n.dot(d - s);


            // TODO: Add the point-to-point constraints to the system
            A.row(idx + 1) << 0, sz, -sy, 1, 0, 0;
            b.row(idx + 1) << dx - sx;

            A.row(idx + 2) << -sz, 0, sx, 0, 1, 0;
            b.row(idx + 2) << dy - sy;

            A.row(idx + 3) << sy, -sx, 0, 0, 0, 1;
            b.row(idx + 3) << dz - sz;
